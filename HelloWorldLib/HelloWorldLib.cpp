...
extern "C" __declspec ( dllexport ) void _stdcall iterate_double_gpu(int* iterations, double ratioXDouble, double ratioYDouble, int currentMaxIterations, int width, int height, double currentMinPlotXDouble, double currentMaxPlotXDouble, double currentMinPlotYDouble, double currentMaxPlotYDouble, double currentBlockSizeXDouble, double currentBlockSizeYDouble)
{
	...
	array_view<int,2> iterationsAV(height, width, &iterations[0]);

    // Run code on the GPU
    parallel_for_each(iterationsAV.extent, [=] (index<2> idx) restrict(amp)
    {
		double dx = (idx[1] * currentBlockSizeXDouble) + currentMinPlotXDouble;
		double dy = currentMaxPlotYDouble - (idx[0] * currentBlockSizeYDouble);
...
extern "C" __declspec ( dllexport ) void _stdcall iterate_partial_double_gpu(int* iterations, int currentMaxIterations, int width, int height, int offsetHeight, double currentMinPlotXDouble, double currentMaxPlotXDouble, double currentMinPlotYDouble, double currentMaxPlotYDouble, double currentBlockSizeXDouble, double currentBlockSizeYDouble)
{
    ...
    {
		double dx = (idx[1] * currentBlockSizeXDouble) + currentMinPlotXDouble;
		double dy = currentMaxPlotYDouble - ((idx[0]+offsetHeight) * currentBlockSizeYDouble);
